class SettingsController < ApplicationController
  ...
  def delete_account
    if @user.try(:authenticate, params[:user][:password].to_s)
      @user.delete!
      reset_session
      flash[:success] = I18n.t('controllers.settings_controller.accountdeleted')
      return redirect_to "/"
    end

    flash[:error] = I18n.t('controllers.settings_controller.passwordnotverified')
    return redirect_to settings_path
  end

  def pushover
    if !Pushover.SUBSCRIPTION_CODE
      flash[:error] = I18n.t('controllers.settings_controller.pushoverdisabled')
  ...
  def pushover_callback
    if !session[:pushover_rand].to_s.present?
      flash[:error] = I18n.t('controllers.settings_controller.norandomtokensession')
      return redirect_to "/settings"
    end

    if !params[:rand].to_s.present?
      flash[:error] = I18n.t('controllers.settings_controller.norandomtokenurl')
      return redirect_to "/settings"
    end
    ...
    if @user.pushover_user_key.present?
      flash[:success] = I18n.t('controllers.settings_controller.pushoverconfigured')
    else
      flash[:success] = I18n.t('controllers.settings_controller.pushoverdeconfigured')
    end
  ...
  def update
    ...
    if @edit_user.update_attributes(user_params)
      flash.now[:success] = I18n.t('controllers.settings_controller.updated')
      @user = @edit_user
    end
